...
type
  ...
  public
    ...
    procedure imprime_pedido(Pedido :TPedido; Departamento :TDepartamento; comandas :String; const cdsparcial :TClientDataSet = nil);
   ...
   for i := 0 to Pedido.Itens.Count - 1 do begin
     ...
     if not (assinou) then begin
       ...
       if not (Pedido.paraEntrega) and not (Pedido.paraRetiradaLocal) then begin
         ...
         comandas := IfThen(comandas <> '',comandas, '');
       end;
     ...
     if assigned(cdsparcial) and not cdsparcial.Locate('CODIGO_ITEM', (Pedido.Itens[i] as TItem).codigo, []) then
       continue;

     if assigned(cdsparcial) then begin
       qtd_item   := cdsparcial.FieldByName('quantidade').AsFloat;
       fracionado := (cdsparcial.FieldByName('fracionado').AsString = 'S')
     end
     else begin
       qtd_item   := (Pedido.Itens[i] as TItem).quantidade;
       fracionado := (Pedido.Itens[i] as TItem).Fracionado = 'S';
     end;

     total_itens   := total_itens + IfThen((Pedido.Itens[i] as TItem).Produto.codigo = 1, (Pedido.Itens[i] as TItem).valor_Unitario * 1,
                                                                               (Pedido.Itens[i] as TItem).valor_Unitario * qtd_item);

     produto    := '';
     produto2   := '';

     produto    := Copy((Pedido.Itens[i] as TItem).Produto.descricao,1,36);
     produto2   := Copy((Pedido.Itens[i] as TItem).Produto.descricao,37,42);

     quantidade := IfThen(((qtd_item > 198)and(fracionado)) or (qtd_item > 599) ,TDateTimeUtilitario.SegundosParaTime(trunc(qtd_item)),
                                                                                 FloatToStr(qtd_item));

     produto    := quantidade +' '+ produto;

     valor := FormatFloat(' ,0.00; -,0.00;', IfThen((Pedido.Itens[i] as TItem).Produto.codigo = 1, (Pedido.Itens[i] as TItem).valor_Unitario * 1,
                                                                                        (Pedido.Itens[i] as TItem).valor_Unitario * qtd_item));
     ...
     if assigned((Pedido.Itens[i] as TItem).Adicionais) then
       for j := 0 to (Pedido.Itens[i] as TItem).Adicionais.Count - 1 do begin

         acao       := IfThen(((Pedido.Itens[i] as TItem).Adicionais[j] as TAdicionalItem).flag = 'A','Adicionar', 'Remover');

         valor_adicional := IfThen(((Pedido.Itens[i] as TItem).Adicionais[j] as TAdicionalItem).flag = 'A',
                                                  ((Pedido.Itens[i] as TItem).Adicionais[j] as TAdicionalItem).valor_unitario,
                                                  0);
         valor_adicional := (valor_adicional * ((Pedido.Itens[i] as TItem).Adicionais[j] as TAdicionalItem).quantidade);
         valor_adicional := valor_adicional * qtd_item;

         total_itens     := total_itens + valor_adicional;

         quantidade := IntToStr(((Pedido.Itens[i] as TItem).Adicionais[j] as TAdicionalItem).quantidade);
         quantidade := IfThen(quantidade='0', '', quantidade);

         materia    := ((Pedido.Itens[i] as TItem).Adicionais[j] as TAdicionalItem).Materia.descricao;
 ...
 try
   ...
   while x < 5 do begin
     ...
     for i := 0 to Pedido.Itens.Count - 1 do begin
       ...
       produto    := '';
       produto2   := '';
       codigo_produto := (Pedido.Itens[i] as TItem).Produto.codigo;

       produto    := intToStr(codigo_produto) +' '+ Copy((Pedido.Itens[i] as TItem).Produto.descricao,1,38);
       produto2   := Copy((Pedido.Itens[i] as TItem).Produto.descricao,39,42);
