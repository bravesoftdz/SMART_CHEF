...
begin
   ...
   NFCe.recibo         := self.FQuery.FieldByName('recibo').AsString;
...
begin
   ...
   if (NFCeAntigo.recibo <> NFCeNovo.recibo) then
     Auditoria.AdicionaCampoAlterado('recibo', NFCeAntigo.recibo, NFCeNovo.recibo);
...
begin
  ...
  Auditoria.AdicionaCampoExcluido('recibo'        , NFCe.recibo);
end;
...
begin
  ...
  Auditoria.AdicionaCampoIncluido('recibo'        , NFCe.recibo);
end;
...
begin
  ...
  self.FQuery.ParamByName('recibo').AsString         := NFCe.recibo;
...
begin
  result := 'update or insert into NFCE ( CODIGO ,NR_NOTA ,CODIGO_PEDIDO ,SERIE ,CHAVE ,PROTOCOLO ,DH_RECEBIMENTO ,XML ,STATUS ,MOTIVO, recibo) '+
           '                     values ( :CODIGO , :NR_NOTA , :CODIGO_PEDIDO , :SERIE , :CHAVE , :PROTOCOLO , :DH_RECEBIMENTO , :XML , :STATUS , :MOTIVO, :recibo) ';
end;
