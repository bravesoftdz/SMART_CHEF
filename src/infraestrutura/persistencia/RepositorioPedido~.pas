...
uses
  ...
  Auditoria,
  Repositorio,
  fabricaRepositorio, StringUtilitario, DateTimeUtilitario, DBClient, funcoes,
 ...
 try
   ...
   if assigned(Pedido.Itens) then begin
     ...
     for i := 0 to Pedido.Itens.Count -1 do begin
       ...
       if self.FQuery.Connection.Database = fdm.ZConnection1.Database then begin
...
begin
   ...
   for i := 0 to Pedido.Itens.Count - 1 do begin
     ...
     quantidade := IfThen(qtd_item < 599 ,FloatToStr(qtd_item),
                                                                   TDateTimeUtilitario.SegundosParaTime(trunc(qtd_item)));
   ...
   while x < 5 do begin
     ...
     for i := 0 to Pedido.Itens.Count - 1 do begin
       ...
       if not (assinou) then begin
         ...
         Writeln(Arq, ConvertAnsiToOem(' USUARIO: ')+TItem(Pedido.Itens[i]).Usuario.Nome);
