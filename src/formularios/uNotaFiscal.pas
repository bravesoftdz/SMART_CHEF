...
uses
  ...
  ConfiguracoesNF, frameBuscaPessoa, frameBuscaCliente, frameBuscaCFOP, frameBuscaFornecedor, frameBuscaProduto, Vcl.Imaging.pngimage,
  frameBuscaNaturezaOperacao;
...
type
  TfrmNotaFiscal = class(TfrmPadrao)
    ...
    BuscaCFOP1: TBuscaNaturezaOperacao;
   ...
   if not Assigned(self.FNotaFiscal) then
    begin
       self.FNotaFiscal := TNotaFiscal.Create(
                                              self.BuscaCFOP1.NaturezaOperacao,
                                              '001',
    ...
    begin
       self.FNotaFiscal.CFOP         := self.BuscaCFOP1.NaturezaOperacao;
   ...
   try
     ...
     try
       ...
       on E: EAccessViolation do
        inherited Avisar('Erro em TfrmNotaFiscal.AdicionarPedidoNaNotaFiscal(const CodigoPedido: Integer)'+#13+
     ...
     not(pos('DIF. PARCIAL CFE ART 108 RICMS', memoObservacoes.Text) > 0) then begin
       ...
       try
          PercentagemReducao := StrToFloat(campo_por_campo('ICMS_ESTADO',
                                                           'PERC_REDUCAO_BC',
                                                           'CODIGO_ESTADO',
                                                           intToStr(NF.Destinatario.Enderecos[0].Cidade.codest) ));
       ...
       Except
         avisar(PERCENTAGEM_REDUCAO_BC_NAO_CADASTRADA + NF.Destinatario.Enderecos[0].Cidade.estado.sigla);
       end;
...
begin
   try
     ...
     if (self.FNotaFiscal.Destinatario.Enderecos[0].Cidade.estado.sigla = 'PR') and
...
begin
  try
     ...
     begin
        icms_por_estado := TIcmsEstado.CreatePorEstado(self.FNotaFiscal.Destinatario.Enderecos[0].Cidade.codest);

        result := icms_por_estado.perc_reducao_bc;
     end;
  except
    on E: EAccessViolation do
      raise Exception.Create(PERCENTAGEM_REDUCAO_BC_NAO_CADASTRADA+self.FNotaFiscal.Destinatario.Enderecos[0].Cidade.estado.sigla);
  end;
end;
...
begin
     ...
     self.BuscaCFOP1.NaturezaOperacao       := NotaFiscal.CFOP;
...
begin
  ...
  try
    ...
    if not assigned(self.FNotaFiscal.ItensAvulsos) and not assigned(BuscaCFOP1.NaturezaOperacao) then
...
begin
  inherited;
  if Assigned(self.FNotaFiscal) and Assigned(self.BuscaCFOP1.NaturezaOperacao) then
    self.FNotaFiscal.CFOP := self.BuscaCFOP1.NaturezaOperacao;
end;
...
begin
   try
     ...
     on E: EInvalidCast do
      inherited Avisar('TfrmNotaFiscal.AposAcharEmpresa. Contate o suporte!');
   end;
