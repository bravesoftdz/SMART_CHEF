...
uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Midas,
  Dialogs, uPadrao, Menus, StdCtrls, AppEvnts, WinSkinData, Buttons, TLHelp32,
  jpeg, ExtCtrls, pngimage, RxNotify, xmldom, XMLIntf, msxmldom, XMLDoc, Contnrs, CriaBalaoInformacao,
  frameBotaoImg, ScktComp, ACBrBase, ACBrDevice, StringUtilitario, Shellapi, RLConsts,
  DB, Grids, DBGrids, DBClient, ZAbstractRODataset, ZAbstractDataset, Printers,
  ZDataset, dblookup, DBCtrls, XPMan;
...
uses uCadastroPerfilAcesso, PermissoesAcesso, uCadastroUsuario, uCadastroGrupo, uCadastroMateriaPrima,
     ...
     uConfiguracoesSistema, uCadastroCliente, uRelatorioProdutos, funcoes;
...
begin
  ...
  try
    ...
    for i := 0 to ListaArquivos.Count - 1 do begin
      ...
      try
         ...
         Pedido.valor_pago      := StrToFloat(StringReplace(vXMLDoc.DocumentElement.ChildNodes['valor_pago'].text,'.',',',[]));
         ...
         for x := 0 to Corpo.ChildNodes.Count - 1 do begin
             ...
             if not assigned(Pedido.Itens) then
               Pedido.Itens := TObjectList.create(true);
  ...
  Except
    On E: Exception do begin
      ...
      if not fdm.conexao.AutoCommit then fdm.conexao.Rollback;
    end;
  end;
      ...
      if Item.Produto.codigo_departamento = dm.UsuarioLogado.Codigo_departamento then begin
        ...
        if assigned(Item.Adicionais) then begin
          for i:= 0 to Item.Adicionais.Count - 1 do
            adicionais := adicionais + IfThen( TAdicionalItem(Item.Adicionais[i]).flag = 'A','+','-') + TAdicionalItem(Item.Adicionais[i]).Materia.descricao;
...
procedure TfrmInicial.pedidosClick(Sender: TObject);
begin
  CriarPedido1Click(nil);
end;

procedure TfrmInicial.caixaClick(Sender: TObject);
begin
...
procedure TfrmInicial.SuporteTcnico1Click(Sender: TObject);
begin
...
begin
  ...
  AppName := PChar('prjFinalizaProcessosSmart') ;
  ShellExecute(0, 'open', AppName, nil, nil, SW_SHOWNORMAL) ;
end;
...
begin
  ...
  CopyFile(PAnsiChar(origem),PAnsiChar(destino),true);
