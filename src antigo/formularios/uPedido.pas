...
uses
  ...
  ZAbstractRODataset, ZAbstractDataset, ZDataset, DB, DBClient, Grids,
  DBGrids, DBGridCBN, pngimage, Buttons, frameBuscaProduto, Produto, Mask,
  ToolEdit, CurrEdit, Pedido, StrUtils, contnrs, Menus, MateriaPrima, Funcoes,
...
type
  ...
  public
    procedure adiciona_item(Produto :TProduto; quantidade, valor :Real; impresso :String; codigo_usu :integer; hora: String; const codigo :integer = 0);
  ...
  private
    procedure Salva_Pedido(const finalizado :boolean = false; const mantem_na_tela :boolean = false);
    procedure Salva_movimento;
...
uses Usuario, Item, Comanda, uModulo, repositorio, FabricaRepositorio, Empresa, uAdicionaItemProduto,
     Math, AdicionalItem, CriaBalaoInformacao, uFinalizaPedido, Movimento, RepositorioPedido, Especificacao,
     ZConnection, Departamento, Estoque, EspecificacaoEstoquePorProduto, uInicial, PermissoesAcesso, ItemDeletado,
     uPesquisaSimples, Venda, ServicoEmissorNFCe, ConfiguracoesSistema, Cliente, Endereco, EspecificacaoEstoquePorItemDispensa,
     ParametrosNFCe, ParametrosDANFE, StringUtilitario, EspecificacaoClientePorCpfCnpj, ProdutoDependenciaEst;
...
procedure TfrmPedido.adiciona_item(Produto: TProduto; quantidade, valor :Real; impresso :String; codigo_usu :integer; hora: String; const codigo :integer);
begin
  ...
  if (Produto.tipo = 'S') and (trunc(quantidade) > 599) then begin //600 = 10min
    cdsItensQUANTIDADE.AsString       := TDateTimeUtilitario.SegundosParaTime(trunc(quantidade));
    cdsItensVALOR.AsCurrency          := valor;
  end
  else begin
    cdsItensQUANTIDADE.AsString       := FloatToStr(quantidade);
    cdsItensVALOR.AsCurrency          := (quantidade * valor);
  end;
...
begin
  ...
  while not cdsItens.Eof do begin
    ...
    try
      total_qtde_itens  := total_qtde_itens + cdsItensQUANTIDADE.AsFloat;
    except
  ...
  while not cdsAdicionais.Eof do begin
    ...
    if cdsAdicionaisFLAG.AsString = 'A' then begin
      ...
      total_adicionados := total_adicionados + ( cdsAdicionaisVALOR.AsFloat * cdsItensQUANTIDADE.AsInteger);
    end;
...
begin
 ...
 try
    ...
    for i := 0 to Pedido.Itens.Count -1 do begin
      ...
      if TItem(Pedido.Itens[i]).Produto.codigo = 1 then
        Item.Produto.valor := TItem(Pedido.Itens[i]).valor_Unitario;
...
procedure TfrmPedido.Salva_Pedido(const finalizado :boolean; const mantem_na_tela :boolean);
...
begin
 ...
 try
   ...
   buscaComanda1.Pedido.situacao        := IfThen(finalizado,'F', 'A');
   ...
   if finalizado then
     case frmFinalizaPedido.cdsMoedasTIPO_MOEDA.AsInteger of
      1 : buscaComanda1.Pedido.tipo_moeda      := '01';
      2 : buscaComanda1.Pedido.tipo_moeda      := '02';
      3 : buscaComanda1.Pedido.tipo_moeda      := '03';
     end;
   ...
   //  testar as formas que passa por aqui.. se sempre que ja tem item que foi salvo, esta puxando o seu codigo
     ...
     try
       Item.quantidade := cdsItensQUANTIDADE.AsInteger;
     Except
     ...
     if not cdsAdicionais.IsEmpty then begin
       ...
       AdicionalItem   := nil;
       Item.Adicionais := TObjectList.Create(true);
   ...
   if buscaComanda1.Pedido.valor_pago > 0 then begin
     buscaComanda1.Pedido.observacoes := buscaComanda1.Pedido.observacoes +'|'+'VALOR TOTAL: R$'+ TStringUtilitario.FormataDinheiro(buscaComanda1.Pedido.valor_total)+
                                         ' TROCO: R$'+TStringUtilitario.FormataDinheiro(buscaComanda1.Pedido.valor_pago - buscaComanda1.Pedido.valor_total);
   end;
   ...
   if buscaComanda1.Pedido.situacao = 'F' then begin
     Salva_movimento;
     Baixa_estoque(buscaComanda1.Pedido);
     imprimir_pedido;
   end;
   ...
   if (buscaComanda1.Pedido.situacao = 'F') and not(frmFinalizaPedido.ckbSC.Checked) and
     ...
     codigo_pedido       := buscaComanda1.Pedido.codigo;

     Caminho_externo     := dm.conexao.Database;

     dm.conexao.Disconnect;
     dm.conexao.Database := dm.ZConnection1.Database;
     ...
     dm.conexao.Disconnect;
     dm.conexao.Database := Caminho_externo;

     if not dm.conexao.Connected then
       dm.conexao.Connect;

     buscaComanda1.Pedido.codigo := codigo_pedido;

     buscaComanda1.Pedido.Itens := nil;
     buscaComanda1.Pedido.Itens;
   end;

   if not finalizado and not mantem_na_tela then
 ...
 Except
   On E: Exception do begin
     ...
     if dm.conexao.Database = dm.ZConnection1.Database then begin
...
begin
  ...
  edtQuantidade.AsInteger          := cdsItensQUANTIDADE.AsInteger;
end;
  ...
  try
    ...
    if frmFinalizaPedido.ShowModal = mrOk then begin
      try
         ...
         if not (frmFinalizaPedido.ckbSC.Checked) and (produtos_no_pedido > 0) then begin
           try
...
procedure TfrmPedido.Salva_movimento;
var repositorio :TRepositorio;
...
begin
  try
    ...
    while not frmFinalizaPedido.cdsMoedas.Eof do begin
      ...
      Movimento.codigo_pedido := buscaComanda1.Pedido.codigo;
      Movimento.data          := now;
 ...
 try
  ...
  // Venda.Acrescimo     := buscaComanda1.Pedido.acrescimo;
   ...
   for i := 0 to Pedido.Itens.Count - 1 do begin
     ...
     if TItem(Pedido.Itens[i]).Produto.tipo = 'S' then begin
       Venda.Total_em_servicos := Venda.Total_em_servicos + ( TItem(Pedido.Itens[i]).valor_Unitario *
                                                              IfThen( TItem(Pedido.Itens[i]).quantidade > 599, 1, TItem(Pedido.Itens[i]).quantidade) );
       Continue;
     end;

     if assigned( TItem(Pedido.Itens[i]).Adicionais ) then
       for x := 0 to TItem(Pedido.Itens[i]).Adicionais.Count - 1 do    //Adicionados
         if TAdicionalItem(TItem(Pedido.Itens[i]).Adicionais[x]).flag = 'A' then
            valor_adicionais := valor_adicionais + ( TAdicionalItem(TItem(Pedido.Itens[i]).Adicionais[x]).valor_unitario *
                                                     TAdicionalItem(TItem(Pedido.Itens[i]).Adicionais[x]).quantidade);

     Venda.AdicionarItem( TItem(Pedido.Itens[i]).codigo_produto,
                          (TItem(Pedido.Itens[i]).valor_Unitario + valor_adicionais),
                          IfThen( TItem(Pedido.Itens[i]).quantidade > 599, 1, TItem(Pedido.Itens[i]).quantidade));
   end;
    ...
    while not cdsAgrupaComanda.Eof do begin
      ...
      if assigned(Pedido_aux) then begin
        ...
        for i := 0 to Pedido_aux.Itens.Count -1 do begin
          Item               := TITem( rep_item.Get( TItem(Pedido_aux.Itens[i]).codigo ));
...
begin
 ...
 try
   ...
   for i := 0 to Pedido.Itens.Count - 1 do begin
     Especificacao := TEspecificacaoEstoquePorProduto.Create( TItem(Pedido.Itens[i]).Produto );
     Estoque       := TEstoque( repositorio.GetPorEspecificacao( Especificacao ) );

     if assigned(Estoque) then begin
       if Estoque.quantidade > 0 then
         Estoque.quantidade := Estoque.quantidade - TItem(Pedido.Itens[i]).quantidade;
     ...
     if assigned(TItem(Pedido.Itens[i]).Produto.ListaDispensa) then begin

       for x := 0 to TItem(Pedido.Itens[i]).Produto.ListaDispensa.count - 1 do begin
          Especificacao := TEspecificacaoEstoquePorItemDispensa.Create( TProdutoDependenciaEst(TItem(Pedido.Itens[i]).Produto.ListaDispensa.Items[x]).dispensa );
          Estoque       := TEstoque( repositorio.GetPorEspecificacao( Especificacao ) );

          if assigned(Estoque) then begin
            if Estoque.quantidade > 0 then
              Estoque.quantidade := Estoque.quantidade - ( TItem(Pedido.Itens[i]).quantidade *
                                    TProdutoDependenciaEst(TItem(Pedido.Itens[i]).Produto.ListaDispensa.Items[x]).quantidade_baixa);
